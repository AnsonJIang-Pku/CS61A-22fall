...
def flatten(s):
    ...
    def my_flatten(lst, ans):
        ...
        return ans
    return my_flatten(s, [])
# or just use sum() literal: sum(s, []), but you should consider how many times sum would be exec.
...
def distance(city_a, city_b):
    ...
    5.0
    """
    lat1, long1 = get_lat(city_a), get_lon(city_a)
    lat2, long2 = get_lat(city_b), get_lon(city_b)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    temp_city = make_city('Target', lat, lon)
    delta_a = distance(temp_city, city_a)
    delta_b = distance(temp_city, city_b)
    if delta_a == delta_b:
...
def sprout_leaves(t, leaves):
    ...
    """
    if is_leaf(t):
        return tree(label(t), [tree(ele) for ele in leaves])
    else:
...
def preorder(t):
    ...
    [2, 4, 6]
    """
    def traversal(t, so_far):
        so_far.append(label(t))
        if is_leaf(t):
            return so_far
        else:
          for b in branches(t):
...
def add_trees(t1, t2):
    ...
    4
      ...
      5
    """
    #Don't violate abstraction barrier!
