...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operands_to_be_evaled = expressions.rest
        if not isinstance(operands_to_be_evaled.first, Pair):
            value = operands_to_be_evaled.first
        else:
            value = scheme_eval(operands_to_be_evaled.first, env)
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        func_name, func_formals = signature.first, signature.rest
        func_body = expressions.rest
        ############
        # Check valid formals:
        # Atom expr in formals
        now = func_formals
        for _ in range(len(func_formals)):
            if not scheme_symbolp(now.first):
                raise SchemeError
            now = now.rest
        ############
        lambda_proc = LambdaProcedure(func_formals, func_body, env)
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        for _ in range(len(expressions)):
            val = scheme_eval(now.first, env)
            if is_scheme_false(val):
                return False
            if _ == len(expressions) - 1:
                return val
            now = now.rest
        return 
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while now is not nil:
        validate_form(now.first, 2, 2)
        names, vals = Pair(now.first.first, names), Pair(scheme_eval(now.first.rest.first, env), vals)
